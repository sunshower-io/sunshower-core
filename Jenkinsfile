    ...
    stages {
        ...
        stage('Release and Deploy Artifacts') {
            ...
            steps {
                ...
                sh 'mvn versions:set-property -Dproperty=sunshower-core.version -DnewVersion=$NEXT_VERSION -f bom/pom.xml'
                /**
                 * Git config
                 */
                 ...
                 * Update remote
                 */
                sh "git remote set-url origin https://${GITHUB_USR}:${GITHUB_PSW}@github.com/sunshower-io/sunshower-core"


                sh 'mvn versions:set -DnewVersion=$NEXT_SNAPSHOT -f bom/pom.xml'
                sh 'mvn versions:set-property -Dproperty=sunshower-core.version -DnewVersion=$NEXT_SNAPSHOT -f bom/pom.xml'
