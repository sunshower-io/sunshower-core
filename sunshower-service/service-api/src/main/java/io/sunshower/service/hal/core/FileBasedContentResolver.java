...
public class FileBasedContentResolver implements ContentResolver {
  ...
  public InputStream read() {
    ...
    if (file != null) {
      try {
        return new BufferedInputStream(new FileInputStream(new java.io.File(file.getPath())));
  ...
  public void write(InputStream value) {
    ...
    if (file != null) {
      ...
      try {
        ...
        topic.onNext(
            TemplateEvents.contentWritten(
                content, entity, new String(data), targetId, targetType, target));
      } catch (IOException e) {
