...
public class GitRepositoryTestCase extends SerializationTestCase {
    ...
    public GitRepositoryTestCase(SerializationAware.Format format, boolean includeRoot, Class[] bound) {
    ...
    public void setUp() {
        ...
        root = new File(
                strategy.resolve(null, null, null),
                UUID.randomUUID().toString()
        );

        repo = new Repository();
        local = new Local();
        local.setFile(new io.sunshower.model.core.io.File(Paths.get(
        ...
        remote.setUri(Paths.get(
                root.getAbsolutePath()
        ).resolve("repo/remote").toString());
