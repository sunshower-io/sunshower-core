...
public class ParallelTaskExecutorTest extends ServiceTestCase {
    ...
    public void ensureLoggingWorks() throws ExecutionException, InterruptedException {
        for(int j = 0; j < 10; j++) {
            ...
            logger.subscribe(new Observer<TaskEvent>() {
                @Override
    ...
    public void ensureSimpleGraphWorks() throws InterruptedException {
        ...
        ExecutionMonitor run = executor.run(
                taskGraph, 
                new TaskContext()
                .bind(int.class).to(101)
        );
        run.join();
    ...
    @Test
    public void ensureInjectingDoesNotInterleveForMultipleNodesOfTheSameType() throws InterruptedException {

        TaskGraph graph = Tasks.newGraph()
                .task("A")
                .bind("io::sunshower::sample-task")
                ...
                .task("C")
                .bind("io::sunshower::successor-task")
                .toContext(new SuccessorSampleNode()).create();
    ...
    public void ensureSingleTaskWithNoDependenciesHasIdInjected() throws InterruptedException {
        TaskGraph graph = copy(Tasks.newGraph()
                .task("B")
                .bind("io::sunshower::sample-task")
                .toContext(new SampleNode("B")).create());
    ...
    public void postConstruct() {
        ...
        context.register(
                "io::sunshower::successor-task",
                SuccessorSampleTask.class
        );

        context.register(
                "io::sunshower::tertiary-task",
                TertiaryDependent.class
        );

        context.register(
                "io::sunshower::sample-task",
                SampleTask.class
        );
    }

    private TaskGraph createTaskGraph(String c) {
        return copy(Tasks.newGraph()
                .task(c)
                .bind("io::sunshower::sample-task")
                .toContext(new SampleNode(c)).create());
    }
