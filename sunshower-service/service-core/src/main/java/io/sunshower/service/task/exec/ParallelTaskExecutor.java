...
public class ParallelTaskExecutor implements Thread.UncaughtExceptionHandler {
  ...
  public ExecutionMonitor run(TaskGraph taskGraph) {
    final ParallelTask task = new ParallelTask(taskGraph, new TaskContext());
    executorService.submit(task);
    return task;
  }

  public ExecutionMonitor createPlan(TaskGraph graph) {
    return new ParallelTask(graph, new TaskContext());
  }
  ...
  private PSchedule createParallelSchedule(
    ...
    GraphTraversal<Vertex, Vertex> group = source.V().where(inE().count().is(eq(0)));
    ...
    while (group.hasNext() && running) {
      LevelSet<TaskElement> set =
          createLevelSet(
              group, task.taskLogger, context, ls, vertices, graph, schedule.id, subject);
      schedule.add(set);
  ...
  private LevelSet<TaskElement> createLevelSet(
      GraphTraversal<Vertex, Vertex> group,
      TaskLogger logger,
      TaskContext context,
      Set<Identifier> ls,
      Map<Identifier, Vertex> vertices,
    ...
    final LevelSet<TaskElement> set =
        new ParallelLevelSet(
            logger, context, scheduleId, graph, subject, executorService, applicationContext, this);
    while (group.hasNext()) {
      Vertex t = group.next();
  ...
  private Triplet<TinkerGraph, TinkerGraph, Map<Identifier, Vertex>> buildGraph(
      TaskGraph taskGraph) {
    log.info("building internal graph from taskGraph");
    final TinkerGraph graph = TinkerGraph.open();
    final TinkerGraph shadowGraph = TinkerGraph.open();
    final Map<Identifier, Vertex> vertices = new LinkedHashMap<>();

    log.info("Computing vertex structure...");
    final Map<Identifier, Vertex> shadowVertices = new LinkedHashMap<>();
    final ContextResolver resolver = new ExpressionContextResolver(graph);
    computeVertices(taskGraph, graph, shadowGraph, taskGraph, resolver, vertices, shadowVertices);

    log.info("Vertex structure complete");
    log.info("Computing edge structure");
    computeEdges(graph, shadowGraph, taskGraph, vertices, shadowVertices);
  ...
  private void computeVertices(
    ...
    for (Node v : result.getNodes()) {
      if (!vertices.containsKey(v.getId())) {
        ...
        final Vertex vertex = graph.addVertex(T.id, v.getId(), T.label, name);
        Vertex shadowVertex = shadowGraph.addVertex(T.id, v.getId(), T.label, name);
  ...
  private void computeEdges(
    ...
    for (Edge edge : result.getEdges()) {
      ...
      Vertex shadowSource =
          shadowVertices.computeIfAbsent(
              edge.getSource(), k -> shadowGraph.addVertex(edge.getSource()));

      Vertex shadowTarget =
          shadowVertices.computeIfAbsent(
              edge.getTarget(), k -> shadowGraph.addVertex(edge.getTarget()));

      Vertex source =
          vertices.computeIfAbsent(edge.getSource(), k -> graph.addVertex(edge.getSource()));

      Vertex target =
          vertices.computeIfAbsent(edge.getTarget(), k -> graph.addVertex(edge.getTarget()));

      final String relationshipKey = elementContext.resolveRelationship(edge, result).getKey();
  ...
  class ParallelTask implements ExecutionMonitor, Runnable {
    ...
    private PSchedule createSchedule() {
      ...
      Triplet<TinkerGraph, TinkerGraph, Map<Identifier, Vertex>> result = buildGraph(taskGraph);
    ...
    public TaskDefinition resolve(Identifier id) {
      ElementDescriptor<?> descriptor =
          (ElementDescriptor<?>) graph.vertices(id).next().property("descriptor").value();
