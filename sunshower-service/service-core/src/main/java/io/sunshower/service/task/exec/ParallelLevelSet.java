...
class ParallelLevelSet implements LevelSet<TaskElement> {
  ...
  void run() {
    ...
    subject.onNext(new TaskEvent(TaskEvent.Type.SequenceBeginning, scheduleId, sequenceId));
    doRun();
    if (last) {
      subject.onNext(new TaskEvent(TaskEvent.Type.SequenceComplete, scheduleId, sequenceId));
