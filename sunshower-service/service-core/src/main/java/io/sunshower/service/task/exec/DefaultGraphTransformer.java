  ...
  public TaskGraph transform(Graph graph) {
    ...
    Map<Identifier, Vertex> vertices =
        graph
            .getVertices()
            .stream()
            .collect(Collectors.toMap(DistributableEntity::getId, identity()));
    Map<Identifier, Edge> edges =
        graph.getEdges().stream().collect(Collectors.toMap(DistributableEntity::getId, identity()));
      ...
      if (!v.getContents().isEmpty()) {
        ...
        List<Edge> adjacencies =
            graph
                .getEdges()
                .stream()
                .filter(t -> t.getSource().getId().equals(v.getId()))
