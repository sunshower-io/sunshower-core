...
public class DefaultConfigurableFileResolutionStrategy implements FileResolutionStrategy {
  ...
  static final Logger logger =
      LoggerFactory.getLogger(DefaultConfigurableFileResolutionStrategy.class);
    ...
    if (!file.exists()) {
      ...
      if (!file.mkdirs()) {
        throw new IllegalStateException(
            String.format(
                "Attempt to " + "create '%s' failed.  " + "I cannot continue",
  ...
  private File configureDefault(Tenant tenant, User user, Credential credential) {
    ...
    final File file =
        new File(property, String.format("sunshower/storage/%s/workspace", user.getId()));
