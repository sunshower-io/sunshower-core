...
import io.sunshower.service.hal.core.Properties;
...
@Service
@Transactional
public class JpaOrchestrationTemplateService extends
        BaseRepository<Identifier, OrchestrationTemplate>
        implements OrchestrationTemplateService {

    public static final String SUMMARY_PATH = "orchestrations/%s/summary.hgs";
    ...
    @Inject
    private GraphSerializationContext graphContext;
    ...
    public JpaOrchestrationTemplateService() {
        super(
    ...
    public Graph getGraph(@P("id") Identifier id, Revision revision) {
        ...
        try (
                GitRepository open = repositoryService.open(ws.getRepository())
        ) {
            open.checkout(revision);
            final String path = String.format(
                    "orchestrations/%s/orchestration.hsf",
                    id
            );
    ...
    public Graph getCurrentGraph(Identifier tid) {
        ...
        try (
            ...
            final String path = String.format(
                    "orchestrations/%s/orchestration.hsf", id
            );
            if (!open.exists(path)) {
    ...
    private void copyProperties(Graph existing, Graph graph) {
        final Map<Identifier, Vertex> existingVertices = 
                existing.getVertices().stream()
                .collect(Collectors.toMap(
                        DistributableEntity::getId, 
                        Function.identity())
                );

        
        final Map<Identifier, Edge> existingEdges =
                existing.getEdges().stream()
                        .collect(Collectors.toMap(
                                DistributableEntity::getId,
                                Function.identity())
                        );
    ...
    public Revision commit(Identifier id) {
        return commit(id, new LineMessage(
                "Saving orchestration template at: " + new Date()
        ));
    }
    ...
    public OrchestrationTemplate get(Identifier id) {
        return super.get(id);
    }
    ...
    public OrchestrationTemplate delete(Identifier id) {
        return super.delete(id);
    }

    @Override
    public OrchestrationTemplate link(Identifier id, OrchestrationTemplate template) {
        final OrchestrationTemplate source = get(id);
        source.getWorkspace().addOrchestrationTemplate(template);
        save(template);
    ...
    @Override
    public OrchestrationTemplate link(Identifier id, Identifier tid) {
        final OrchestrationTemplate source = get(id);
        final OrchestrationTemplate target = get(tid);
    ...
    private WorkspaceRef resolveWorkspace(Identifier templateId) {
        ...
        OrchestrationTemplate                              template = get(templateId);
        Link<OrchestrationTemplate, OrchestrationTemplate> link     = template.getLink();
        while (link != null) {
    ...
    static class WorkspaceRef {
        ...
        final OrchestrationTemplate template;

        WorkspaceRef(Workspace workspace, OrchestrationTemplate template) {
