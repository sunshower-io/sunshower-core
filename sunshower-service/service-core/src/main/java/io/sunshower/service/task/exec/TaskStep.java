...
class TaskStep implements Callable<ExecutionResult> {
    ...
    private void fromPreconditions(Class<?> type, Object value, Map<Class<?>, Object> result) {
        ...
        if (type.isAnnotationPresent(Preconditions.class)) {
            ...
            for (Pair<Class<?>, Object> precondition : preconditions) {
                ...
                runPrecondition(
                        precondition.getValue0(),
                        precondition.getValue1(),
                        result
                );
            }
        }
    ...
    private Pair<Class<?>, Object>[] compute(Preconditions annotation) {
        ...
        @SuppressWarnings("unchecked") final Pair<Class<?>, Object>[] results = new Pair[value.length];
        Arrays.sort(value, (lhs, rhs) -> lhs.order() < rhs.order() ? -1 : 1);
        for (int i = 0; i < value.length; i++) {
            results[i] = Pair.with(
                    value[i].condition(),
                    createPrecondition(value[i].condition())
            );
        }
        return results;
    }
    ...
    public ExecutionResult<?> call() {
        ...
        try {
            ...
            ParallelTaskExecutor.log.log(Level.WARNING, "Failed to execute task {0} in plan {1}.  " +
                            "Reason: {2}.  Full trace at debug",
                    new Object[]{
                            resolveId(),
                            scheduleId,
                            e.getMessage()
...
//                ParallelTaskExecutor.log.log(Level.WARNING, "Caught unexpected exception while processing result", ex);
//            }
//        }
    }
                ...
                if (f.isAnnotationPresent(Context.class)) {
                    ...
                    if (value != null) {
                        try {
                            ...
                            ParallelTaskExecutor.log.log(Level.WARNING,
                                    "Failed to set field {0} on type {1}." +
                                            "  Did you declare it final?",
