...
public class JGitRepository implements GitRepository {
  ...
  public JGitRepository(final Repository repository, final User session) {
    this.session = session;
    this.repository = repository;
    this.local = Paths.get(repository.getLocal().getFile().getPath()).toFile();
  ...
  public Lock lock() {
    check();
    try (RandomAccessFile file = new RandomAccessFile(local, "w"); ) {
  ...
  public void checkout(Revision commit) {
    ...
    try {
      RevCommit revCommit =
          new RevWalk(git.getRepository()).parseCommit(ObjectId.fromString(commit.getRevision()));
      git.checkout().setCreateBranch(true).setName("working").setStartPoint(revCommit).call();
    } catch (Exception e) {
      throw new RepositoryException(e);
    }
  }
