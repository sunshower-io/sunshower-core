...
public class JpaContentHandler implements ContentHandler {
    ...
    @Override
    public ContentHandler setProperties(String name, PropertyInclusion property, Collection<Property<?, ?>> properties) {
        check(property);
        switch(property) {
            case Graph:
        ...
        results.addAll(graph.getVertices()
                ...
                .flatMap(t ->
                        Stream.concat(
                                ...
                                t.getContents()
                                        .stream()
                                        .flatMap(u -> u.getProperties().stream())
                        )
                ).collect(Collectors.toSet()));
        results.addAll(graph.getEdges()
                .stream().flatMap(t -> 
                        Stream.concat(
                                t.getProperties().stream(),
                                t.getContents()
    ...
    public ContentResolver resolve(Content content) {
        ...
        if (element.getContents().stream().noneMatch(t -> t.equals(content))) {
            throw new IllegalStateException(String.format("Content %s does not exist in this graph", content));
        }
    ...
    private void create(Content content) {
        ...
        try {
            ...
            throw new RuntimeException(String.format(
                    ...
                    path.toFile().getAbsolutePath(), parentFile.getAbsolutePath(), parentFile.exists(), ex.getMessage())
            );
        }
    }
