...
public class UUIDEnabledLookupStrategy implements LookupStrategy {
     ...
     */
    public UUIDEnabledLookupStrategy(DataSource dataSource, AclCache aclCache,
    ...
    private void lookupPrimaryKeys(final Map<Serializable, Acl> acls,
                                   final Set<UUID> findNow, final List<Sid> sids) {
        Assert.notNull(acls, "ACLs are required");
        Assert.notEmpty(findNow, "Items to find now required");

        String sql = computeRepeatingSql(lookupPrimaryKeysWhereClause, findNow.size());

        Set<UUID> parentsToLookup = jdbcTemplate.query(sql,
                ps -> {
                    int i = 0;
