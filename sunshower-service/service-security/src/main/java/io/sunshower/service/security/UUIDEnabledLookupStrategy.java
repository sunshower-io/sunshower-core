...
/**
 * Pretty much copied verbatim from Spring's implementation. It's beyond me why they do not support
 * UUIDs OOTB
 */
public class UUIDEnabledLookupStrategy implements LookupStrategy {
   ...
   */
  public UUIDEnabledLookupStrategy(
  ...
  private void lookupPrimaryKeys(
      final Map<Serializable, Acl> acls, final Set<UUID> findNow, final List<Sid> sids) {
    Assert.notNull(acls, "ACLs are required");
    Assert.notEmpty(findNow, "Items to find now required");

    String sql = computeRepeatingSql(lookupPrimaryKeysWhereClause, findNow.size());

    Set<UUID> parentsToLookup =
        jdbcTemplate.query(
            sql,
            ps -> {
              int i = 0;

              for (UUID toFind : findNow) {
                i++;
                ps.setObject(i, toFind);
              }
            },
  ...
  private Map<ObjectIdentity, Acl> lookupObjectIdentities(
        ...
        jdbcTemplate.query(
            ...
            ps -> {
              ...
              for (ObjectIdentity oid : objectIdentities) {
                ...
                UUID identifier = (UUID) oid.getIdentifier();
    ...
    for (Acl inputAcl : acls.values()) {
      ...
      Acl result = convert(acls, (UUID) ((AclImpl) inputAcl).getId());
   ...
   */
  private AclImpl convert(Map<Serializable, Acl> inputMap, UUID currentIdentity) {
  ...
  /**
   * The SQL for the select clause. If customizing in order to modify column names, schema etc, the
   * other SQL customization fields must also be set to match.
   *
  ...
  private class ProcessResultSet implements ResultSetExtractor<Set<UUID>> {
    ...
    public Set<UUID> extractData(ResultSet rs) throws SQLException {
      Set<UUID> parentIdsToLookup = new HashSet<>(); // Set of parent_id Longs
      ...
      while (rs.next()) {
        ...
        UUID parentId = (UUID) rs.getObject("parent_object");
    ...
    private void convertCurrentResultIntoObject(Map<Serializable, Acl> acls, ResultSet rs)
        throws SQLException {
      UUID id = fromBytes(rs.getObject("acl_id"));
      ...
      if (acl == null) {
        ...
        ObjectIdentity objectIdentity =
            new ObjectIdentityImpl(
                rs.getString("class"), (UUID) rs.getObject("object_id_identity"));

        Acl parentAcl = null;
        UUID parentAclId = (UUID) rs.getObject("parent_object");
      ...
      if (rs.getString("ace_sid") != null) {
        ...
        UUID aceId = (UUID) rs.getObject("ace_id");
  ...
  static UUID fromBytes(Object bytes) {
    return (UUID) bytes;
  }
}
