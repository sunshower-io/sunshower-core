...
public class UUIDJdbcMutableAclService extends JdbcAclService implements MutableAclService {
  ...
  public UUIDJdbcMutableAclService(
  ...
  protected UUID createOrRetrieveClassPrimaryKey(String type, boolean allowCreate) {
    List<UUID> classIds =
        jdbcTemplate.queryForList(selectClassPrimaryKey, new Object[] {type}, UUID.class);
   ...
   * @return the primary key or null if not found
   */
  protected UUID createOrRetrieveSidPrimaryKey(
    ...
    if (allowCreate) {
      final UUID id = UUID.randomUUID();
      jdbcTemplate.update(insertSid, id, Boolean.valueOf(sidIsPrincipal), sidName);
      Assert.isTrue(
          TransactionSynchronizationManager.isSynchronizationActive(),
          "Transaction must be running");
      return id;
  ...
  protected UUID retrieveObjectIdentityPrimaryKey(ObjectIdentity oid) {
    ...
    try {
      return jdbcTemplate.queryForObject(
          selectObjectIdentityPrimaryKey, UUID.class, oid.getType(), oid.getIdentifier());
