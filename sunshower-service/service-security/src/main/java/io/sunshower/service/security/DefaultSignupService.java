...
public class DefaultSignupService implements SignupService {
    ...
    public RegistrationRequest signup(User user) {
        ...
        final Role role = roleService.findOrCreate(
                DefaultRoles.TENANT_USER.toRole());
    ...
    private void executePostSignup(User user) {
        Map<String, Object> beansWithAnnotation = applicationContext
                .getBeansWithAnnotation(PostSignupListener.class);
        for (Map.Entry<String, Object> bean :
    ...
    private void performPostSignup(
            Map.Entry<String, Object> bean,
            User user
    ) {
        ...
        ReflectionUtils.doWithMethods(type, m -> {
            ...
            if(t.isAnnotationPresent(PostSignup.class)) {
                if(t.getParameterCount() == 1 &&
                        t.getParameterTypes()[0]
                                .equals(User.class)) {
                    return true;
                } else {
                    throw new IllegalStateException(
                            "PostSignup method requires " +
                            "exactly 1 parameter of type user");
                }
            }
            return false;
        });
    ...
    public User approve(String s) {
        final List<RegistrationRequest> u = entityManager.createQuery(
                "select r from " +
        ...
        ).setParameter("id", s).getResultList();
        if (u.size() == 1) {
    ...
    public List<RegistrationRequest> pendingRegistrations() {
        return entityManager.createQuery(
                ...
                RegistrationRequest.class).getResultList();
    }
}
